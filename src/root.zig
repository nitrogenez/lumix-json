...
fn AggregateContainerStack(comptime n: usize) type {
    return struct {
        ...
        pub fn push(self: *Self, ty: AggregateContainerType) ?void {
            ...
            const sub_index: ElementShiftAmountType = @intCast(self.len % element_bitcount);
        ...
        pub fn peek(self: *Self) ?AggregateContainerType {
            ...
            return @as(AggregateContainerType, @enumFromInt(bit));
        }
...
pub const StreamingParser = struct {
    ...
    pub const State = enum(u8) {
        ...
        pub fn fromAggregateContainerType(ty: AggregateContainerType) State {
            ...
            return @as(State, @enumFromInt(@intFromEnum(ty)));
        }
    };
                    ...
                    .String => |stringToken| {
                        ...
                        inline for (structInfo.fields, 0..) |field, i| {
                            ...
                            if ((stringToken.escapes == .None and mem.eql(u8, field.name, key_source_slice)) or (stringToken.escapes == .Some and (field.name.len == stringToken.decodedLength() and encodesTo(field.name, key_source_slice)))) {
                                ...
                                if (field.is_comptime) {
                                    if (!try parsesTo(field.field_type, @as(*align(1) const field.field_type, @alignCast(@ptrCast(field.default_value.?))).*, tokens, child_options)) {
            ...
            inline for (structInfo.fields, 0..) |field, i| {
                if (!fields_seen[i]) {
                    if (field.default_value) |default_ptr| {
                        if (!field.is_comptime) {
                            const default = @as(*align(1) const field.field_type, @ptrCast(default_ptr)).*;
